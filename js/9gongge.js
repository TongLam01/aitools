...
【视觉风格与主题】
极简新年马年情侣风格。以马年新年祝福语作为背景装饰元素，排版留白充足。整体氛围自然、温馨、有生活感。

【主体人物描述】
九宫格内每一格都必须包含一男一女，且保持高度一致性。
将参考图1（男性）和参考图2（女性）作为画面主角。
- 参考图1为男性：黑色短发，身穿米白色（或浅灰色）针织毛衣，肤感干净。
- 参考图2为女性：黑色长发，身穿红色针织一字领毛衣，妆容清透。
★重要约束：画面中必须且只能出现一男一女，严禁出现两个男性或两个女性。

【九宫格详细动作与文字配置】
1. 第一格：(男左女右) 一男一女并肩站立，手拿节日装饰棒。背景文字：「新年好」。
2. 第二格：(男左女右) 一男一女同时做出贴脸手势，头部靠近。背景文字：「马年大吉」。
3. 第三格：(男左女右) 两人中的一人手举“福”字牌，另一人温柔注视。背景文字：「马到成功」。
4. 第四格：(男左女右) 两人并排坐姿，肩膀轻触，手拿红包。背景文字：「一马当先」。
5. 第五格（中心位）：(男左女右) 两人牵手站立，背景浅红。背景文字：「新年吉祥」。
6. 第六格：(男左女右) 轻松玩闹姿态，手拿红包。背景文字：「Happy 2026」。
7. 第七格：(男左女右) 两人身体微倾，动作同步。背景文字：「喜气」。
8. 第八格：(男左女右) 一人举拳拜年，一人微笑。背景文字：「马到成功」。
9. 第九格：(男左女右) 两人贴近，手拿福气球。背景文字：「马上发财」。

【核心要求与约束】
始终保持一男一女的构图。利用提供的两张参考图特征，准确还原服装颜色（男米白、女红）和发型差异。禁止出现多余人物。禁止出现页头文本或标题`
};
...
window.switchMode = function(mode) {
    ...
    if (mode === 'male') {
        ...
        text1.innerText = "📸 上传男生照片 (图1)"; 
        document.getElementById('text-2').innerText = "📸 上传女生照片 (图2)"; 
...
window.onload = function() {
    ...
    // ★★★ 核心修改：读取本地缓存的 API Key ★★★
    const cachedKey = localStorage.getItem('volc_api_key');
    if (cachedKey) {
        document.getElementById('api-key').value = cachedKey;
        logStatus("🔑 已自动填入缓存的 API Key");
    }
...
async function generateImage() {
    ...
    try {
        ...
        const payload = {
            ...
            // 保持 disabled，这是告诉模型“虽然我给了两张图，但我要合成一张结果”
